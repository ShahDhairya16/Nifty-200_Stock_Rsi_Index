...
class ExcelReportService:
    """Generate Excel reports from local CSV/JSON market data."""

    @staticmethod
    def generate_nifty200_excel_report(output_dir: Optional[Path] = None, refresh_market_data: bool = True,
                                       end_date: Optional[date] = None) -> Dict[str, Any]:
        report_end = parse_date(end_date) if end_date is not None else date.today()
        prices = FileStore.load_prices()
        prices = prices[prices["trade_date"] <= report_end]
        if prices.empty:
            raise ValueError("No local market data found. Run --sync-universe and --backfill-historical first.")
        ...
        rsi = rsi[rsi["trade_date"] <= report_end]
        ...
        ranking = ranking.rename(columns={"trade_date": "Latest Date"})[["Rank", "symbol", "company_name", "Latest Date", "rsi_22", "rsi_44", "rsi_66", "average_rsi"]]
        ...
        for ws in workbook.worksheets:
            ...
            for column in ws.columns:
                ws.column_dimensions[column[0].column_letter].width = min(max(max(len(str(cell.value or "")) for cell in column) + 2, 12), 38)
        target_dir = output_dir or OUTPUT_DIR
        target_dir.mkdir(parents=True, exist_ok=True)
        filename = f"NIFTY200_RSI_Report_{report_end.isoformat()}_{datetime.now():%H%M%S}.xlsx"
        path = target_dir / filename
        workbook.save(path)
        return {"success": True, "file_path": str(path), "filename": filename, "latest_trade_date": format_date_iso(max(dates)), "trading_days": len(dates), "stocks": len(symbols)}
